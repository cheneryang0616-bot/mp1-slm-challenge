...
def main():
    ...
    # Optional recipe knobs. Defaults reproduce the original recipe exactly, so
    # omitting these flags (as the baseline command does) is bit-identical.
    p.add_argument('--lr', type=float, default=1e-3,
                   help='Peak learning rate (original recipe: 1e-3).')
    p.add_argument('--weight-decay', type=float, default=0.1,
                   help='AdamW weight decay (original recipe: 0.1).')
    p.add_argument('--warmup', type=int, default=100,
                   help='Linear warmup steps (original recipe: 100).')
    args = p.parse_args()
    ...
    for step in range(args.steps):
        ...
        learning_rate = args.lr * min(1.,(step+1)/args.warmup) * (.1+.9*.5*(1+math.cos(math.pi*step/args.steps)))
